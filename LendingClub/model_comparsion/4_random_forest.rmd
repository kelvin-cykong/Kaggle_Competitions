---
title: "STAT652 Project 4. Random Forest"
...
Setting up formula for random forest algorithms:
...
pred <- pred %>%
  ...
  rename(loan_status_rf = .pred_class)
```

```{r}
confusion_rf <- pred %>%
  conf_mat(truth = loan_status, estimate = loan_status_rf)
...
```{r}
pred %>%
  accuracy(loan_status, loan_status_rf)
```

# Step 5 Improving Model Performance
We can improve the random forest model by changing the number of trees.